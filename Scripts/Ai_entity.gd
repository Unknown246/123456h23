...
export var patrol_max_distance : float = 900
export var chase_timeout_time : float = 0.2
export var what : float = 0.5
...
func update_patrol_target():
	var pre_target := position + Vector2(rand_range(0, patrol_max_distance), 0).rotated(deg2rad(rand_range(0, 360)))
...
func new_state(new_value):
	...
	if new_value == EntityState.PATROL:
		...
		patrol_timeout_timer.start(what)
